...
SRC := src/main.c src/util.c src/db.c src/json.c src/token_store.c \
...
build/%: tests/%.c src/util.c src/db.c src/json.c src/token_store.c $(wildcard src/core/*.c) $(wildcard src/ui/*.c)
	@mkdir -p build
	$(CC) $(CFLAGS) $^ -o $@ $(LDFLAGS)
